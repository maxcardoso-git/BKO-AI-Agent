...
### Validated (v1.0 - 2026-03-18)

All 111 v1 requirements complete and shipped:
- Foundation, Auth, Tickets, Orchestration, AI/KB, 19 Skills, HITL, Memory, Personas, Admin, Observability, LGPD
- 16-step pipeline operational with HITL pause at compliance_check
- Frontend bko-console deployed; backend serving from PM2 on 72.61.52.70:3111

## Current Milestone: v2 - Operator Workflow

**Goal:** Simplificar UX de revisão para usuário final via bloco de notas e fluxo aprovar/reprovar/corrigir que alimenta o aprendizado da IA. Remover dependência de tabelas externas (faturas/descontos) do pipeline.

**Target features:**
- Tela `/processar` com busca por protocolo + bloco de notas (campos estruturados + texto livre) para dados complementares
- Pipeline simplificado: 14 steps (remove `retrieve_discounts`, `retrieve_invoices`)
- Tela de validação simplificada com 3 ações: Aprovar / Corrigir / Reprovar
- Feedback de correção/rejeição alimenta `HumanFeedbackMemory` para training da IA
- Tela técnica de pipeline (atual) restrita a ADMIN/SUPERVISOR
...
---
*Last updated: 2026-04-10 after milestone v2 start*
