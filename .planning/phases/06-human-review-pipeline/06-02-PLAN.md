...
must_haves:
  truths:
    ...
    - "Artifacts from the current execution are displayed in the artifact panel (type label + JSON content)"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    export function StepProcessor({ complaint, execution, artifacts }: Props) {
      ...
      // Find current artifact (last artifact from current step, or last overall)
