...
import { StepExecution } from '../../execucao/entities/step-execution.entity';
...
export class StepsDesignerService {
   ...
   */
  async getCapabilityVersion(verId: string): Promise<any> {
    ...
    // Enrich steps with skill bindings
    const bindingRepo = this.dataSource.getRepository(StepSkillBinding);
    const enrichedSteps = await Promise.all(
      (ver.steps ?? []).map(async (step) => {
        const binding = await bindingRepo.findOne({
          where: { stepDefinitionId: step.id, isActive: true },
          relations: ['skillDefinition'],
        });
        return {
          ...step,
          skillKey: binding?.skillDefinition?.key ?? null,
          llmModel: binding?.llmModel ?? null,
        };
      }),
    );

    return { ...ver, steps: enrichedSteps };
  }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        if (existing) {
          ...
          if ((dto as any).posX !== undefined) (existing as any).posX = (dto as any).posX;
          if ((dto as any).posY !== undefined) (existing as any).posY = (dto as any).posY;
  ...
  async listVersionSelectors(): Promise<
    { id: string; capabilityKey: string; capabilityName: string; version: number; description: string | null; isActive: boolean; isCurrent: boolean; stepCount: number }[]
  > {
  ...
  async activateVersion(verId: string): Promise<{ success: boolean }> {
    ...
    if (!ver) throw new HttpException('Version not found', 404);

    await this.capVersionRepo.update(
  ...
  /**
   * Returns output variable keys from the most recent step execution for a given stepKey.
   * Used by the frontend condition editor to populate variable dropdowns.
   */
  async getStepOutputVariables(stepKey: string): Promise<{ key: string; type: string; sampleValue: string }[]> {
    const stepExecRepo = this.dataSource.getRepository(StepExecution);
    const latest = await stepExecRepo.findOne({
      where: { stepKey },
    ...
    const excludeKeys = ['usage', 'model', 'provider', 'latencyMs', 'artifactId'];
   ...
   */
  async toggleVersionActive(verId: string, isActive: boolean): Promise<{ success: boolean }> {
    const ver = await this.capVersionRepo.findOne({ where: { id: verId } });
    if (!ver) throw new HttpException('Version not found', 404);
    await this.capVersionRepo.update({ id: verId }, { isActive });
    return { success: true };
  }

  /**
   * Clones a capability version: creates a new version with copied steps and transitions.
   */
  async cloneVersion(verId: string): Promise<{ id: string; version: number }> {
    const source = await this.capVersionRepo.findOne({
      where: { id: verId },
      relations: ['steps', 'capability'],
    });
    if (!source) throw new HttpException('Version not found', 404);

    return this.dataSource.transaction(async (manager) => {
      const capVerRepo = manager.getRepository(CapabilityVersion);
      const stepDefRepo = manager.getRepository(StepDefinition);
      const bindingRepo = manager.getRepository(StepSkillBinding);
      const transitionRepo = manager.getRepository(StepTransitionRule);

      // Find max version number for this capability
      const maxResult = await capVerRepo
        .createQueryBuilder('v')
        .select('MAX(v.version)', 'maxVer')
        .where('v.capabilityId = :capId', { capId: source.capabilityId })
        .getRawOne();
      const newVersionNumber = (maxResult?.maxVer ?? 0) + 1;

      // Create new version
      const newVer = await capVerRepo.save(
        capVerRepo.create({
          capabilityId: source.capabilityId,
          version: newVersionNumber,
          description: source.description ? source.description + ' (clone)' : 'Clone da v' + source.version,
          isActive: false,
          isCurrent: false,
        }),
      );
      ...
      // Copy steps
      const stepKeyToNewId = new Map<string, string>();
      for (const step of (source.steps ?? []).sort((a, b) => a.stepOrder - b.stepOrder)) {
        const newStep = (await stepDefRepo.save(
          stepDefRepo.create({
            capabilityVersionId: newVer.id,
            key: step.key,
            name: step.name,
            stepOrder: step.stepOrder,
            isHumanRequired: step.isHumanRequired,
            isActive: step.isActive,
          }),
        )) as StepDefinition;
        stepKeyToNewId.set(step.key, newStep.id);

        // Copy skill bindings
        const bindings = await bindingRepo.find({ where: { stepDefinitionId: step.id } });
        for (const binding of bindings) {
          await bindingRepo.save(
            bindingRepo.create({
              stepDefinitionId: newStep.id,
              skillDefinitionId: binding.skillDefinitionId,
              llmModel: binding.llmModel,
      ...
      // Copy transitions
      const oldStepIds = (source.steps ?? []).map((s) => s.id);
      if (oldStepIds.length > 0) {
        const oldStepKeyMap = new Map((source.steps ?? []).map((s) => [s.id, s.key]));
        const oldTransitions = await transitionRepo
          .createQueryBuilder('tr')
          .where('tr.stepDefinitionId IN (:...ids)', { ids: oldStepIds })
          .getMany();

        for (const tr of oldTransitions) {
          const fromKey = oldStepKeyMap.get(tr.stepDefinitionId);
          const newStepId = fromKey ? stepKeyToNewId.get(fromKey) : undefined;
          if (newStepId) {
            await transitionRepo.save(
              transitionRepo.create({
                stepDefinitionId: newStepId,
                targetStepKey: tr.targetStepKey,
                conditionType: tr.conditionType,
